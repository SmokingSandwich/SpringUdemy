...
class ProjectServiceTest {
    ...
    @Test
    @DisplayName("Should throw IllegalArgumentException when configured to allow just 1 group and no groups and no project")
    ...
    void createGroup_configurationOk_existingProject_createsAndSavesGroup() {
        ...
        var mockRepository = mock(ProjectRepository.class);
        when(mockRepository.findById(anyInt())).thenReturn(
                Optional.of(project));
        //and
        ...
        assertThat(result.getTasks().stream().allMatch(task -> task.getDescription().equals("foo"))); //Added stream here
    ...
    private static class InMemoryGroupRepository implements TaskGroupRepository {
        ...
        public boolean existsByDescription(String description) {
            map.values().stream().anyMatch(group -> !group.getDescription().equals(description));
            return false;
        }
    }
}